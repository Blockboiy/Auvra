  ...
  return <>
    ...
    <div className="grid gap-6 xl:grid-cols-[1.1fr_.9fr]">
      ...
      <div className="space-y-6">
        ...
        <section className="card p-5 sm:p-6"><p className="eyebrow">Phase 2 · User-approved transfers</p><h2 className="mt-2 font-semibold text-ink">Wallet-connected resources and transfers</h2><div className="mt-4 flex gap-2 text-xs leading-5 text-muted"><ShieldCheck className="mt-0.5 h-4 w-4 shrink-0 text-emerald-600" /> Browser wallet connection, live CREDIT/ETH/USDG reads, exchange deployment checks, funding target planning and explicit user-approved CREDIT transfers are available under Resources.</div><div className="mt-3 flex gap-2 text-xs leading-5 text-muted"><AlertCircle className="mt-0.5 h-4 w-4 shrink-0 text-amber-600" /> Wallet signing is permitted only for an individually reviewed CREDIT transfer. Funding readiness is read-only; activated inference balance, activation, purchases and automatic top-ups remain disabled until Orbio contract and account attribution are verified.</div><a href="/app/resources" className="button-secondary mt-5">Open Resources</a></section>
      </div>
    </div>
    <div className="mt-6 rounded-2xl border border-line bg-white px-5 py-4 text-xs leading-5 text-muted"><strong className="text-ink">Security notice:</strong> the Railway demo uses a shared password, not individual user accounts. Wallet reads happen in the visitor's browser and are not bound to an Auvra account. Browser wallet transfers require an explicit signature by the wallet owner. Browser-local transaction history is not shared across users or devices. Do not enable shared server-side wallet funding or unattended transactions without account isolation and a security review.</div>
  </>;
}
